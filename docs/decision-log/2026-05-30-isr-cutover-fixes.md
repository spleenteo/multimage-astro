...
**Follow-up (2026-05-31)**: the `sanitized_html` validator was still present on more fields than this entry claimed — the earlier removal had either been re-applied or never reached primary. A full schema sweep (`schema:inspect`) found it on **7 of 16** WYSIWYG fields: `supplier.description`, `book.description`, `home.claim`, `home.cta_body`, `home.newsletter_body`, `cta_button_with_image.content`, `index_page.subtitle`. It was hitting the editor again — with `sanitize_before_validation: false` it doesn't loop but **blocks the save** outright ("Il campo non può contenere codice HTML pericoloso", e.g. on the Campagna Soci banner CTA body). Confirmed the rule both ways: validator ON + sanitize OFF = save blocked on "dangerous" content; validator ON + sanitize ON = dirty-form loop; there is no validator state that both saves freely and never loops. **Decision**: removed `sanitized_html` from all WYSIWYG fields (direct `client.fields.update` via CLI, no migration; `required` preserved on `home.claim` and `cta_button_with_image.content`). Anti-spurious-HTML protection now lives **exclusively** at the editor/toolbar layer for every WYSIWYG field. New policy: **no WYSIWYG field should carry the `sanitized_html` validator** — if spurious HTML is a concern, tighten the toolbar (and drop `show_source`), don't add a save/validation-time sanitizer.
