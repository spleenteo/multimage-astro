...
Guidelines and long-lived policies now live under `/docs/guidelines` (read-only via `agent_edit: false`). Editable inventories and work-in-progress docs remain at the root of `/docs`.
...
- **How to manage blocks and components:** `/docs/guidelines/cms-content-modelling.md`
...
- **DatoCMS naming conventions:** `/docs/guidelines/cms-content-modelling.md`
...
- **Read-only:**
  - All files with `agent_edit: false` (CI will enforce). This includes every document under `/docs/guidelines/**`.
- **Never touch:**
  - `docs/DATOCMS.md`
